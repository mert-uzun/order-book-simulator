...
void Metrics::on_fill(long long order_id_1, long long fill_price_ticks, long long fill_timestamp_us, int filled_quantity, bool was_instant) {
    ...
    if increasing position
        update position, update avg_price_per_share

    if decreasing position
        update position, update realized_pnl, handle flips
